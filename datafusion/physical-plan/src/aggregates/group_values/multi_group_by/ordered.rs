...
mod tests {
    ...
    #[test]
    fn ordered_fixed_binary_timestamp_keys_match_hash_grouping() -> Result<()> {
        // The actual metrics keys include nullable buckets. Null handles also
        // exercise the Arrow equality contract admitted by the type gate.
        let rows = [
            (None, None),
            (None, None),
            (None, None),
            ...
            (Some([1_u8; 16]), None),
            (Some([1_u8; 16]), None),
            (Some([1_u8; 16]), Some(30)),
            (Some([1_u8; 16]), Some(30)),
            (Some([1_u8; 16]), Some(60)),
            (Some([2_u8; 16]), None),
            (Some([2_u8; 16]), None),
            (Some([2_u8; 16]), Some(30)),
        ];
        let input: Vec<ArrayRef> = vec![
            Arc::new(FixedSizeBinaryArray::try_from_sparse_iter_with_size(
                rows.iter()
                    .map(|(handle, _)| handle.as_ref().map(|v| &v[..])),
                16,
            )?),
            Arc::new(TimestampNanosecondArray::from_iter(
                rows.iter().map(|(_, bucket)| *bucket),
            )),
        ];
        let schema = Arc::new(Schema::new(vec![
            Field::new("handle", DataType::FixedSizeBinary(16), true),
            Field::new(
                "bucket",
                DataType::Timestamp(TimeUnit::Nanosecond, None),
                true,
        ...
        for batch_size in [1, 2, 4, 7] {
            for emit_limit in [0, 1, usize::MAX] {
                let mut ordered = GroupValuesOrdered::try_new(Arc::clone(&schema))?;
                let mut hashed = GroupValuesColumn::<true>::try_new(Arc::clone(&schema))?;
                let mut actual = Vec::new();
                let mut expected = Vec::new();
                for offset in (0..rows.len()).step_by(batch_size) {
                    let length = batch_size.min(rows.len() - offset);
                    let batch = input
                        .iter()
                    ...
                    assert_eq!(
                        actual, expected,
                        "batch={batch_size}, offset={offset}, emit={emit_limit}"
                    );
                    assert_eq!(ordered.len(), hashed.len());
                    let empty = batch.iter().map(|c| c.slice(0, 0)).collect::<Vec<_>>();
                    ordered.intern(&empty, &mut actual)?;
                    assert!(actual.is_empty());
                    let emit = emit_limit.min(ordered.len().saturating_sub(1));
                    assert_eq!(
